...
import { CreateVehicleInput, UpdateVehicleInput } from './dto';
...
export class VehiclesService {
   ...
   async getCustomerVehicles(input: { user: JwtUser; pagination: PaginationInputDto }) {
   ...
   async createVehicle(input: { user: JwtUser; body: CreateVehicleInput }) {
      const {
         user: { id },
         body
      } = input;
   ...
   async updateVehicle(input: { body: UpdateVehicleInput; id: string }) {
      const { body, id } = input;
